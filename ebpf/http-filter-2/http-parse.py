...
def printUntilCRLF(s,layerInfo):
    # print(s)
    try:
         responseData = s.split(b'\r\n')
         print("========================")
         print("HTTP Request Detected:")
         print("URL: "+responseData[0].decode("utf-8"))
         print("Source IP: {}".format(layerInfo["ip_src"]))
         print("Dest IP: {}".format(layerInfo["ip_dst"]))
         print("Source Port: {}".format(layerInfo["port_src"]))
         print("Dest Port: {}".format(layerInfo["port_dst"]))
         
        #  print("Dest IP: "+layerInfo["ip_dst"])
        #  print("Source Port: "+layerInfo["port_src"])
        #  print("Dest Port: "+layerInfo["port_dst"])
         print(responseData[len(responseData)-1].decode('utf-8')) 
    except:
...
while 1:
    ...
    layerInfo = {"port_src":port_src,"port_dst":port_dst,"ip_src":ip_src,"ip_dst":ip_dst}
    ...
    if ((payload_string[:3] == b'GET') or (payload_string[:4] == b'POST')
            ...
            or (payload_string[:6] == b'DELETE') or (payload_string[:4] == b'HEAD') or (payload_string[:5] == b'HTTPS')):
        # match: HTTP GET/POST packet found
        if (crlf in payload_string):
            # url entirely contained in first packet -> print it all
            printUntilCRLF(payload_string,layerInfo)
        ...
        if (current_Key in bpf_sessions):
            ...
            if (binascii.hexlify(current_Key) in local_dictionary):
                ...
                if (crlf in payload_string):
                    ...
                    printUntilCRLF(prev_payload_string,layerInfo)
